...
for file in "/home/allenwang/gad_diff/gad_lggm/outputs/reddit_anomaly_marginal/checkpoints/reddit_anomaly_marginal/"*; do
    file=$(echo "$file" | sed 's/=/\\=/g')
    echo "file: ${file}"
    # Check if the file ends with '.ckpt'
    if [[ "$file" == *".ckpt" ]]; then
        # Run the Python command with the current file
        python3 main.py dataset.name="${domain}" general.name="aug_${domain}" model.transition="marginal" general.wandb="disabled" general.ckpt_path="$file" train.batch_size=12 general.setting='augment' 
    fi
done
echo "============================================================================================================"